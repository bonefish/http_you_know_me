...
class RequestTest < MiniTest::Test
  ...
  def setup
    ...
    @request.make_request_hash
  end

  def test_request_hash_is_not_empty
    refute @request.request_hash.empty?
  end
  ...
  def test_request_hash_contains_correct_keys
    ...
    assert_equal expected, @request.request_hash.keys
  end
  ...
  def test_request_hash_contains_correct_values
    ...
    assert_equal expected, @request.request_hash.values
  end

  def test_verb_and_path_hash_pair_are_correct
    assert_equal "Verb", @request.request_hash.key("GET")
    assert_equal "Path", @request.request_hash.key("/")
  end

  def test_finds_content_length
    assert_equal 32, @request.content_length
  end
